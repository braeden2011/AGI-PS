    ...
    def do_photogrammetry(self):
        ...
        def update_output():
            for line in iter(process.stdout.readline, b''):
                self.output_text.insert(tk.END, line.decode())
                self.output_text.see(tk.END)

        process = Popen(["python", "agi_and_PS_process.py", self.project_folder, str(self.topo_csv), str(self.camera_cal), str(self.gcp_csv)], stdout=PIPE, stderr=PIPE)
        self.output_thread = Thread(target=update_output)
