...
if len(csv_files) == 1:
    ...
    if gcp_file:
        ...
        print("GCP's found in csv and GCP.csv written. GCP feature not currently avaliable, for future release. Press enter to continue without GCP")
...
#checks if there are 2 CSVs, and one ends in gcp.csv
    ...
    csvPath = os.path.join(output_folder, next(f for f in csv_files if not f.lower().endswith('gcp.csv')))
    filter_and_write_csv(csvPath, image_folder, projectName + "topo", ['sh', 'SH', 'PSHT', 'psht'], 4)
    topoCSVPath = os.path.join(input_directory, projectName, projectName + " topo.csv")
    gcp_file = next((f for f in files if f.lower().endswith('gcp.csv')), None)
    gcpPath = os.path.join(os.path.join(input_directory, projectName), gcp_file)
    print("GCP.csv found. GCP feature not currently avaliable, for future release. Press enter to continue without GCP")
    #Do you want to use these GCP prompt? set GCP use FLAG here
    input()
...
if len(applications) == 1 and "PointStudio" in ''.join(map(str, applications)):
  ...
  print("Surface creation and QA will occur in the most recently opened Pointstudio / Vulcan instance, please make sure the most recently opened is the desired Pointstudio database and press enter.")
  input()
...
project = Project()
pointcloud_Path = "working/projects/" + projectName + "/" 
PS_topo_Path = "working/projects/" + projectName + "/" 
